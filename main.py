#Partie1 : Importation des librairies de Python nécessaires.
import numpy as np
import pandas as pd
...
#Partie2 : Chargement des données ou bien simulation aléatoire des données de dimension supérieure à 4
data = np.random.rand(100, 5)

#Partie3 :Visualisation et taille des données.
print("Taille des données : ", data.shape)

#Partie4 : Implémentation de l’algorithme K-moyennes avec les stratégies d’initialisation des centres suivantes
#a- Aléatoire
...
#Partie5 : Implémentation des méthodes de validation de Clustering.
...
#Partie9 : a- La nouvelle matrice des observations.
pca = PCA(n_components=2)
data_pca = pca.fit_transform(data)

#b- Affichage des valeurs propres et des vecteurs propres associés aux axes principaux.
...
#d- Vérification de la somme des inerties de chaque axe par rapport a la dimension de la base de données.
print("Somme des inerties : ", np.sum(pca.explained_variance_ratio_))

#e- Représentation des données ainsi que des centres obtenus par l’algorithme du k-means sur les deux axes principaux.
